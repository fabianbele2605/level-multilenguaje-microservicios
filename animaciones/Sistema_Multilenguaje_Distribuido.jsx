...
import { Server, Shield, Activity, Cpu, Database, Terminal, Play, Zap, Globe, ArrowRight, Code } from 'lucide-react';
...
const SystemArchitecture = () => {
  ...
  const animatePacket = async (path) => {
    ...
    // Initial Setup
    addLog(`🚀 Iniciando flujo: ${path.name}`, 'system');
    ...
    for (let i = 0; i < path.steps.length; i++) {
      ...
      // Animate transition (simple linear interpolation for demo)
      const frames = 20;
    ...
    addLog("✅ Flujo completado exitosamente", 'success');
  };
  ...
  const flows = {
    ...
    heavy: {
      ...
      steps: [
        { from: 'client', to: 'python', log: 'POST /heavy-analyze' }, // Simplified entry for demo
      ...
      <div className="p-4 border-b border-slate-800 bg-slate-900 flex justify-between items-center shadow-lg z-10">
        <div>
          <h1 className="text-xl font-bold flex items-center gap-2">
            <span className="text-2xl">🚀</span> Sistema Multilenguaje Distribuido
          </h1>
          ...
          {Object.values(services).map((svc) => {
            ...
            return (
              ...
              >
                ...
                {/* Detail Tooltip (always visible if active) */}
                <div className={`
